...
import Mo from './MovieCard/movieCard';
...
const movieList = () => {
    ...
    return (
        ...
        {moviesData.map(el=><Mo  key={el.id} image={el.image} rating={el.rating} name={el.name} date={el.date} type={el.type} description={el.description} />)}    
        </div>
    )
}
